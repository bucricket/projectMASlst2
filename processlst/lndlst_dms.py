...
def perpareDMSinp(productIDpath, s_row, s_col, locglob, ext):
    ...
    meta = landsat_metadata(productIDpath)
...
def getSharpenedLST(productIDpath, sat):
    ...
    if not os.path.exists(tifFile):
        ...
        Parallel(n_jobs=njobs, verbose=5)(delayed(localPred)(productIDpath, th_res, s_row, s_col) for s_col in
                                          range(0, int(ncols / wsize) * wsize, wsize) for s_row in
                                          range(0, int(nrows / wsize) * wsize, wsize))
        ...
        for s_col in range(0, int(ncols / wsize) * wsize, wsize):
            for s_row in range(0, int(nrows / wsize) * wsize, wsize):
                ...
                if os.path.exists(fn):
                    Lg = gdal.Open(fn)
                    globalData[0, s_row * scale:s_row * scale + wsize * scale + 1,
                    s_col * scale:s_col * scale + wsize * scale + 1] = \
                    Lg.ReadAsArray(s_col * scale, s_row * scale, wsize * scale + 1, wsize * scale + 1)[0]
